...
# this is "global" i.e. can be accessed by all routes
...
def stream_hop_data(destination, source=False):
    ...
    try:    
        ...
        # needs toshow this error and go back to the home page
    ...
    # At the end return a button that jumps to /plot map and uses hop_list to plot the map
...
def get_lat_long_center(hop_list):
    ...
    return lat_center, lon_center, zoom_level
                

@app.route('/plot_map', methods=["GET", "POST"])
def plot_map():
    if request.method == "GET" or request.method == "POST": # what else would it be?
        ...
        center_lat, center_lon, zoom_level = get_lat_long_center(hop_list)

        # make a basemap, starting point
        map = folium.Map(
            location=[center_lat, center_lon], # lat/long New York, NY
            zoom_starts=12,
        ...
        for hop    in hop_list:
            if hop["latitude"] != "N/A":
                # if the hop has no lat/long, skip it
                # with only 4 digits, this means they are pretty close
                if hop['latitude'] == last_lat and hop['longitude'] == last_lon:
                    continue
                valid_hops.append([float(hop["latitude"]), float(hop["longitude"])])
                last_lat = hop['latitude']
                last_lon = hop['longitude']
        ...
        for i, hop in enumerate(hop_list):
            ...
            text = (f"Hop #{i}: IP Addr: {str(hop['ip'])}\n" +
                    f"Location: {str(hop['city'])}, {str(hop['region'])}, {str(hop['country'])}\n")


            # plot mop
            folium.CircleMarker(
                location=[float(hop["latitude"]), float(hop["longitude"])], # need to convert to float1
        ...
        return render_template("map_plus_table.html", html_map=html_map, table=hop_list)
